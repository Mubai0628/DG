...
describe("desktop source boundaries", () => {
  ...
  it("documents the fixed multi-agent execution ADR and implementation gate", async () => {
    ...
    const runtimeRunPlan = await readFile(
    ...
    const runtimeRoleAdapters = await readFile(
    ...
    expect(runtimeRunPlan).toContain("Runtime Fixed Agent Run Plan");
    expect(runtimeRunPlan).toContain("does not execute agents");
    expect(runtimeRunPlan).toContain("dynamic bidding");
    ...
    expect(runtimeRoleAdapters).toContain("Runtime Fixed Agent Role Adapters");
    expect(runtimeRoleAdapters).toContain("do not call models");
    expect(runtimeRoleAdapters).toContain("Unknown roles");
  ...
  it("documents the P1A desktop observer ADR and implementation gate", async () => {
    ...
    const profileDoc = await readFile(
      path.join(
        repoRoot,
        "docs",
        "runtime-desktop-observation-profile-v0.22.md"
      ),
      "utf8"
    );
    ...
    expect(profileDoc).toContain("Runtime Desktop Observation Profile v0.22");
    expect(profileDoc).toContain("data contract only");
    expect(profileDoc).toContain("does not observe the desktop");
    expect(profileDoc).toContain("does not observe the desktop, call Tauri");
    expect(profileDoc).toContain("raw screenshot persistence");
    expect(profileDoc).toContain("raw OCR text persistence");
    expect(profileDoc).toContain("send-to-model flags");
